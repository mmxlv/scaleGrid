class ScaleGridLayer extends CanvasLayer {
    ...
    switch (curGridType) {  // This switch was added to determine what type of grid is in use and then apply the correct adjustment calculations.
      ...
      case 1:
        ...
        if (absTopL > absTopR) {
          ...
          sgLayer.printOperationLog("X Offset", xOff);
        } else {
    ...
    switch (curGridType) {  // This switch was added to determine what type of grid is in use and then apply the correct adjustment calculations.
      ...
      case 1:
        ...
        if (absTop < absBot) {
          ...
          sgLayer.printOperationLog("Y Offset", yOff);
        } else {
  ...
  async setGrid(s) {
    ...
    if (sgLayer.currentTool == "3x3") {
      gridPix = gridPix / 3;
    }

    // Ropund the value before passing to Foundry because it will error out on a non-integer.
    ...
    if (gridPix >= 50) {
      ...
      sgLayer.printOperationLog("scale", gridPix);
    }
  ...
  async setHexGrid(s) {
    ...
    if (gridPix >= 50) {
      this.drawChild.clear();
      await curScene.update({ grid: gridPix });      //this will update the current scene, this time it is the grid square size
      sgLayer.printOperationLog("hex scale", gridPix);
    }
    if (gridPix < 50) {
      sgLayer._removeListeners();
      this.drawChild.clear();
  ...
  printOperationLog(desc, num) {
    sgLayer.printLog(`The grid ${desc} was set to ${num}.`);
  }
  ...
  /**
   * Renders a dialog to confirm the user wants to adjust the scale.
   */
